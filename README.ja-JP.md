...
- 現在のバージョン: **v0.7.0**（DisplayVersion `0.7.0` / PackageVersion `0.7.0.0`）
...
- **フローティング残高ウィンドウ**（v0.7.0）: 軽量で常に手前に表示される小さなウィンドウ。選択中の 1 アカウントの主残高の数字だけを大きく表示し（アカウント名・Provider・単位・状態・最終更新は小さく表示）、タスクバーには表示されず、閉じてもアプリは終了しません。アカウントカードの**「フローティングウィンドウに設定」**またはトレイメニューから開き、最後の位置・サイズ・選択アカウントを復元します（旧コンパクトウィンドウの設定は自動移行）。旧コンパクトウィンドウを置き換えます。
