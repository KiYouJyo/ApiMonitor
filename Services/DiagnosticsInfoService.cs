...
/// DisplayVersion、PackageVersion、分发渠道、架构、Windows 版本、.NET 版本、
/// Windows App SDK 版本、Package Identity/Package Family、数据 schema 版本、
/// Provider ID 列表、账户数量、通知注册状态、StartupTask 状态、语言与主题。
/// v1.0.0：渠道来自 IDistributionChannelService（构建时确定）；身份一致性只输出
/// “匹配/不匹配”布尔值，不输出 Credential Locker Resource、API Key 等敏感内容。
...
public sealed class DiagnosticsInfoService : IDiagnosticsInfoService
{
    ...
    public async Task<string> BuildAsync(CancellationToken cancellationToken)
    {
        ...
        sb.AppendLine($"DistributionChannel: {_channel.CurrentChannel}");
        ...
        sb.AppendLine($"PackageIdentity: {AppInfo.PackageIdentity}");
        sb.AppendLine($"PackageFamily: {(string.IsNullOrEmpty(AppInfo.PackageFamilyName) ? "(unpackaged)" : AppInfo.PackageFamilyName)}");
        sb.AppendLine($"ChannelIdentityMatchesInstalledPackage: {_channel.InstalledIdentityMatchesChannel}");
