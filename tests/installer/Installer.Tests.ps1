...
try {
    ...
    $msixName = 'ApiMonitor_0.7.0.11_x64.msix'
    $goodManifest = @'
<?xml version="1.0" encoding="utf-8"?>
<Package xmlns="http://schemas.microsoft.com/appx/manifest/foundation/windows10">
  <Identity Name="ApiMonitor" Publisher="CN=ApiMonitorDev" Version="0.7.0.11" ProcessorArchitecture="x64" />
  <Dependencies>
...
'@
    ...
    Assert-Equal '0.7.0.11' $info.Version '包版本 = 0.7.0.11'
    ...
    $wrongPublisher = @{ Name = 'ApiMonitor'; Version = '0.7.0.11'; Publisher = 'CN=WrongPublisher' }
    Assert-False (Assert-ManifestIdentity $wrongPublisher).Ok '错误 Publisher 被拒绝'
    $wrongName = @{ Name = 'ApiMonitorOther'; Version = '0.7.0.11'; Publisher = 'CN=ApiMonitorDev' }
    ...
    Assert-Equal 'Install' (Resolve-PackageAction $null '0.7.0.11' 'CN=ApiMonitorDev') '未安装 -> Install'
    Assert-Equal 'Upgrade' (Resolve-PackageAction (New-InstalledPkg '0.4.0.0') '0.7.0.11' 'CN=ApiMonitorDev') '低版本允许原地升级'
    Assert-Equal 'Upgrade' (Resolve-PackageAction (New-InstalledPkg '0.6.0.1') '0.7.0.11' 'CN=ApiMonitorDev') 'v0.6.0.1 允许原地升级到 0.7.0.11'
    Assert-Equal 'SameVersion' (Resolve-PackageAction (New-InstalledPkg '0.7.0.11') '0.7.0.11' 'CN=ApiMonitorDev') '相同版本不重复安装'
    Assert-Equal 'HigherVersionInstalled' (Resolve-PackageAction (New-InstalledPkg '0.7.0.12') '0.7.0.11' 'CN=ApiMonitorDev') '更高版本拒绝降级'
    Assert-Equal 'Conflict' (Resolve-PackageAction (New-InstalledPkg '0.7.0.11' 'CN=SomeoneElse') '0.7.0.11' 'CN=ApiMonitorDev') '同名不同 Publisher 判定为冲突'
...
'@
    ...
    Assert-Equal '0.7.0.11' $manifest.appVersion '清单应用版本正确'
    ...
    New-Item -ItemType Directory -Path $emptySource | Out-Null
        $emptyBackup = Backup-SafeLocalState -Source $emptySource -BackupRoot (Join-Path $script:TempRoot 'empty-bak') -PackageFamilyName 'ApiMonitor_cx0n152q1hsh2' -AppVersion '0.7.0.11'
    ...
    [System.IO.File]::WriteAllText((Join-Path $corruptSource 'accounts.json'), '{ broken !!!', (New-Object System.Text.UTF8Encoding($false)))
        $corruptBackup = Backup-SafeLocalState -Source $corruptSource -BackupRoot (Join-Path $script:TempRoot 'corrupt-bak') -PackageFamilyName 'ApiMonitor_cx0n152q1hsh2' -AppVersion '0.7.0.11'
    ...
    [System.IO.File]::WriteAllText((Join-Path $zeroSource 'tray-settings.json'), '', (New-Object System.Text.UTF8Encoding($false)))
        $zeroBackup = Backup-SafeLocalState -Source $zeroSource -BackupRoot (Join-Path $script:TempRoot 'zero-bak') -PackageFamilyName 'ApiMonitor_cx0n152q1hsh2' -AppVersion '0.7.0.11'
    ...
    $flowOps['AddAppxPackage'] = {
        ...
        $script:InstalledPackages += New-InstalledPkg '0.7.0.11'
        $true
    }
    ...
    Assert-Equal 0 $upgradeCode 'v0.6.0.1 -> v0.7.0.11 原地升级成功'
