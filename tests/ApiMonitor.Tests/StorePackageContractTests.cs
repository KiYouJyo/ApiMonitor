...
/// <summary>
/// v1.1.0：Store 包配置契约测试。
/// Store manifest 使用 Partner Center 官方身份；版本 1.1.0.0；
...
public sealed class StorePackageContractTests
{
    ...
    [Fact]
    public void StoreManifest_UsesOfficialIdentity_AndVersion1100()
    {
    ...
    [Fact]
    public void DirectoryBuildProps_ChannelDefaults_KeepStoreVersionFixedAt1100()
    {
        string props = File.ReadAllText(Path.Combine(RepoRoot, "Directory.Build.props"));

        Assert.Contains("<ApiMonitorDisplayVersion>1.1.0</ApiMonitorDisplayVersion>", props);
        Assert.Contains("1.1.0.0", props);
