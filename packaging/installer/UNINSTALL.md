# ApiMonitor v0.9.0 卸载说明（Uninstall.cmd，正式发布包版本 0.9.0.0）

卸载与安装一样简单：双击 **`Uninstall.cmd`** 并按提示操作。

> v0.9.0 中关闭主窗口默认仅隐藏到通知区域；卸载前建议先右键托盘图标选择**“退出 ApiMonitor”**，让脚本能干净结束进程。
...
- 账户 JSON、余额历史与设置位于应用 LocalState 中；原地升级（v0.7.0 → v0.9.0.0）会保留这些数据，卸载则不会保留。
