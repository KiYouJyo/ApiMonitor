# ApiMonitor v0.5.0 安装说明（Install.cmd）

ApiMonitor v0.5.0 是自签名侧载版本。完整 `Test.zip` 解压后，普通用户只需：

1. **下载完整 Test.zip**（`ApiMonitor_0.5.0.0_x64_Test.zip`）并解压到任意目录（路径可包含空格和中文）。
2. **双击 `Install.cmd`**。
...
3. 从 MSIX 提取签名证书，与随包 CER 的**完整 Thumbprint** 比对；核对 Subject 为 `CN=ApiMonitorDev`、代码签名 EKU、有效期、Manifest Publisher 与 Identity（`ApiMonitor` / 0.5.0.0）。
...
6. 全新安装或**原地升级**（v0.4.0 → v0.5.0），保留本地账户、历史、阈值、窗口/托盘/启动设置与 Credential Locker 数据；相同版本不重复安装；更高版本拒绝降级。
...
# 3. 安装主包
Add-AppxPackage -Path .\ApiMonitor_0.5.0.0_x64.msix
```
...
```powershell
Get-FileHash .\ApiMonitor_0.5.0.0_x64.msix -Algorithm SHA256
Get-FileHash .\ApiMonitor_0.5.0.0_x64_Test.zip -Algorithm SHA256
```
