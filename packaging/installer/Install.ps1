...
<#
  ApiMonitor v0.7.0 sideload installer (candidate package version 0.7.0.11)
  ====================================
  ...
  3. Integrity verification:
     ...
     - manifest Publisher matches the certificate Subject;
      - manifest Identity = ApiMonitor and version = 0.7.0.11.
...
[CmdletBinding()]
param(
    [string]$PackageVersion = '0.7.0.11',
...
function Assert-ManifestIdentity {
    param(
        ...
        [string]$ExpectedVersion = '0.7.0.11'
    )
...
function Invoke-Install {
    ...
    Write-InstallerLog '==== ApiMonitor v0.7.0 自动安装开始 ===='
    ...
    switch ($action) {
        ...
        'HigherVersionInstalled' {
            Write-InstallerLog (
                '已安装更高版本（{0}），v0.7.0 安装程序不会执行降级。' -f $installedPkg.Version) 'ERROR'
