...
- 当前版本：**v0.7.0**（DisplayVersion `0.7.0` / PackageVersion `0.7.0.0`）
...
- **悬浮余额窗**（v0.7.0）：轻量、始终置顶的小窗口，只显示一个选定账户的主额度数字（账户名、Provider、单位、状态与最近更新时间以小字展示）。不在任务栏单独占位，关闭不退出应用，全局仅一个实例；可从任意账户卡片（**设为悬浮窗**）或托盘菜单打开，上次位置、尺寸与选中账户自动恢复（旧紧凑窗口设置自动迁移）。替代原紧凑窗口。
...
1. 下载 `ApiMonitor_0.7.0.0_x64_Test.zip`。
