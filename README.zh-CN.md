...
- **v0.6.0** 在 v0.5.0 之上**原地升级**：账户、AccountId、Credential Locker API Key、最新余额、历史记录、阈值、自动刷新设置、通知设置、紧凑窗口设置、托盘设置与登录启动设置全部保留。安装程序**不会**自动开启系统提醒，也**不会**自动开启登录启动。
- **v0.5.0** 在 v0.4.0 之上原地升级（历史版本）。
