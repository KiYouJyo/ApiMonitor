...
- 当前版本：**v0.7.0**（DisplayVersion `0.7.0` / PackageVersion `0.7.0.3`）
...
- **悬浮余额窗**（v0.7.0）：轻量、黑白主题、圆角小方块样式的始终置顶信息块，显示账户名、Provider、主额度数字、单位和简短状态。不在任务栏单独占位，关闭不退出应用，全局仅一个实例；可从任意账户卡片（**设为悬浮窗**）或托盘菜单打开，上次位置与选中账户自动恢复（旧紧凑窗口设置自动迁移）。替代原紧凑窗口。
- 主页顶部已重新整理：标题、操作按钮、提示条与账户概览整体上移，不再为旧副标题保留空白；自定义 `ApiMonitor.ico` 也已显式应用到标题栏窗口图标。
...
1. 下载 `ApiMonitor_0.7.0.3_x64_Test.zip`。
